...
export const startDateTimeSchema = () =>
  z
    ...
    .describe("Start datetime of the event - ALWAYS REQUIRED for both REGULAR and ALL_DAY events");
...
export const endDateTimeSchema = () =>
  z
    ...
    .describe("End datetime of the event - REQUIRED when isStartOnly=false, OPTIONAL when isStartOnly=true. For ALL_DAY events, endDateTime is always required");

export const eventTypeSchema = () =>
  z
    .enum(["REGULAR", "ALL_DAY"])
    .describe("Event type that determines validation rules: 'REGULAR' = event with specific start/end times (can be modified by isAllDay flag start-00:00, end-23:59), 'ALL_DAY' = event spanning full days without time specifications (only date matters)");
...
export const isStartOnlySchema = () => z.boolean().describe("When true, the event only has a start time (no end time). When false, both start and end times are required. This field controls whether the 'end' parameter is mandatory.");

export const isAllDaySchema = () => z.boolean().describe("Only applies to REGULAR events. When true, the event spans the entire day but still requires specific start/end times (start-00:00, end-23:59). This is different from ALL_DAY event type which doesn't require time specifications.");
...
export const watcherSchema = () =>
  z.object({
    ...
    id: idSchema().describe("Unique identifier for the watcher (organization id, user id, or role id)"),
    code: z.string().describe("Code for the watcher (organization code, user code, or role code)"),
    name: z.string().describe("Name for the watcher (organization name, user name, or role name)"),
  }
