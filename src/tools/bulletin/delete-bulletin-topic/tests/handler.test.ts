...
describe("deleteBulletinTopicHandler", () => {
  ...
  it("should successfully delete a bulletin topic", async () => {
    ...
    const result = await deleteBulletinTopicHandler(input, {} as any);

    expect(mockDeleteRequest).toHaveBeenCalledWith("/api/v1/bulletin/topics/1");
  ...
  it("should use correct endpoint with topic ID", async () => {
    ...
    await deleteBulletinTopicHandler(input, {} as any);

    expect(mockDeleteRequest).toHaveBeenCalledWith(
      "/api/v1/bulletin/topics/999",
    );
  });
});
